...
import { useGetFollowersList, useGetUserProfile, useGetProfilePicture } from '../hooks/useQueries';
...
function FollowerItem({ followerPrincipal, onNavigate }: FollowerItemProps) {
  const { data: profile, isLoading: profileLoading } = useGetUserProfile(followerPrincipal);
  const { data: pictureBytes, isLoading: pictureLoading } = useGetProfilePicture(followerPrincipal);
  ...
  const shortPrincipal =
    followerPrincipal.toString().slice(0, 8) + '…' + followerPrincipal.toString().slice(-4);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {!isLoading && (
          <p className="text-xs text-muted-foreground font-mono truncate">{shortPrincipal}</p>
        )}
      </div>
...
export default function FollowersModal({ user, open, onClose }: FollowersModalProps) {
  const navigate = useNavigate();
  const { data: followers, isLoading } = useGetFollowersList(open ? user : undefined);

  const handleNavigate = (principal: Principal) => {
    onClose();
    navigate({ to: '/profile/$principal', params: { principal: principal.toString() } });
  };
