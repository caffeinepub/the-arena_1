...
function AvatarBubble({ principal, size = 'sm' }: { principal: Principal; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={`${sizeClass} rounded-full overflow-hidden border border-arena-neon/20 bg-arena-neon/10 flex items-center justify-center flex-shrink-0`}>
      {avatarUrl ? (
        <img src={avatarUrl} alt={profile?.name ?? 'User'} className="w-full h-full object-cover" />
      ) : (
...
export default function ConversationView({ partnerPrincipal }: ConversationViewProps) {
  ...
  const { data: partnerProfile, isLoading: partnerProfileLoading } = useGetUserProfile(partnerPrincipal);
  ...
  useEffect(() => {
    ...
    messages.forEach((msg, index) => {
      if (!msg.isRead && msg.recipient.toString() === callerPrincipal.toString()) {
        ...
        markAsRead.mutate({
          conversationKey,
          messageIndex: BigInt(index),
          partnerStr: partnerPrincipal.toString(),
        });
      }
    });
      ...
      <ScrollArea className="flex-1 px-4 py-4">
        {messagesLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full py-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Say hello to {partnerName}!</p>
          </div>
        ) : (
