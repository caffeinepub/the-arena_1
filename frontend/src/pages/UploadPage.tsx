...
export default function UploadPage() {
  ...
  const [albumCoverBuffer, setAlbumCoverBuffer] = useState<ArrayBuffer | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      contentBlob = contentBlob.withUploadProgress((pct) => setUploadProgress(pct));
      ...
      const albumCoverBlob = albumCoverBuffer
        ? ExternalBlob.fromBytes(new Uint8Array(albumCoverBuffer as ArrayBuffer))
        : null;
  ...
  return (
    <main className="min-h-screen bg-background py-10 px-4">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-display text-3xl text-arena-neon tracking-wide mb-1">Upload Content</h1>
          <p className="text-foreground/50 text-sm">Share your music or video with the Arena community</p>
        </div>
        ...
        <div className="flex gap-3 mb-8">
          <button
            type="button"
            onClick={() => { setCategory('audio'); setMediaFile(null); setThumbnailBuffer(null); }}
          ...
          <button
            type="button"
            onClick={() => { setCategory('video'); setMediaFile(null); setThumbnailBuffer(null); }}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-200 ${
              !isAudio
                ? 'bg-arena-neon text-arena-dark shadow-neon'
                : 'bg-arena-surface text-foreground/60 hover:text-foreground hover:bg-arena-surface/80'
