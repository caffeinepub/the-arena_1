...
export default function MediaPlayer({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: track.id is the intentional trigger
  useEffect(() => {
    setIsPlaying(false);
    setCurrentTime(0);
    setDuration(0);
  ...
  const handlePlayPause = useCallback(() => {
    ...
    if (isPlaying) {
      ...
      el.play().catch(() => {
        setMediaError(
          "Unable to play this media. The format may not be supported by your browser.",
        );
      });
    }
  ...
  const handleError = useCallback(
    (e: React.SyntheticEvent<HTMLMediaElement>) => {
      ...
      if (errCode === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED) {
        ...
        message =
          "A network error occurred while loading the media. Please try again.";
