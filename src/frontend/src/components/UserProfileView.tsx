...
export default function UserProfileView({
  ...
  const { data: pictureBytes, isLoading: pictureLoading } =
    useGetProfilePicture(targetPrincipal);
  ...
  const shortPrincipal = `${targetPrincipal.toString().slice(0, 8)}…${targetPrincipal.toString().slice(-4)}`;

  const handleFollow = () => {
    followUser.mutate(targetPrincipal, {
      onSuccess: () =>
        toast.success(`Now following ${resolvedName ?? shortPrincipal}`),
      onError: (err: unknown) => {
        const msg =
  ...
  const handleUnfollow = () => {
    unfollowUser.mutate(targetPrincipal, {
      onSuccess: () =>
        toast.success(`Unfollowed ${resolvedName ?? shortPrincipal}`),
      onError: (err: unknown) => {
        const msg =
            ...
            {pictureLoading ? (
              ...
              <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-arena-surface bg-arena-neon/10 flex items-center justify-center shadow-neon">
                {avatarUrl ? (
                  ...
                  <span className="text-2xl font-bold text-arena-neon">
                    {initials}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-4 mt-3">
                {profileLoading ? (
                  ...
                  >
                    ...
                    <span className="group-hover:text-arena-neon transition-colors">
                      Followers
                    </span>
                  </button>
                )}
