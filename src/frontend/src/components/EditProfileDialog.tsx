...
export default function EditProfileDialog({
  ...
  useEffect(() => {
    if (currentPictureBytes && currentPictureBytes.length > 0) {
      const blob = new Blob([new Uint8Array(currentPictureBytes)], {
        type: "image/jpeg",
      });
  ...
  const displayedPreview =
    picturePreview ?? (removePicture ? null : storedPictureUrl);
