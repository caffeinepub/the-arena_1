...
function AuthorAvatar({ principal }: { principal: Principal }) {
  ...
  const avatarSrc = pictureBytes
    ? URL.createObjectURL(
        new Blob([new Uint8Array(pictureBytes)], { type: "image/jpeg" }),
      )
    : undefined;
...
function ThoughtCommentItem({
  ...
  const displayName =
    authorProfile?.name ?? `${authorPrincipalStr.slice(0, 8)}…`;
        ...
        >
          <MessageCircle
            className={`w-4 h-4 transition-all ${
              commentsExpanded
                ? "fill-arena-neon/20 text-arena-neon"
                : "group-hover:text-arena-neon"
            }`}
          />
...
function Composer({ currentPrincipalStr }: { currentPrincipalStr: string }) {
  ...
  const canSubmit =
    (content.trim().length > 0 || mediaFile !== null) && !isOverLimit;
      ...
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-arena-surface border border-arena-border rounded-2xl p-5"
            >
